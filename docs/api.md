...
指定したゲームセッションの戦闘マップサイズ（幅・高さ）を取得します。
...
指定したゲームセッションの戦闘マップサイズを更新します。セッション作成者のみ操作可能です。
...
| フィールド | タイプ | 必須 | 説明 |
|------------|--------|------|------|
| `map_width` | integer | ❌ | マップの幅（1〜99、デフォルト: 24） |
| `map_height` | integer | ❌ | マップの高さ（1〜99、デフォルト: 32） |
